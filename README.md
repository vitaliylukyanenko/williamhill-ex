...
```console
helm repo add app https://app.github.io/helm-charts
helm repo update
```
...
| `extraContainers`                         | Sidecar containers to add to the app pod  | `""`                                                    |
...
| `plugins`                                 | Plugins to be loaded along with app       | `[]`                                                    |
| `datasources`                             | Configure app datasources (passed through tpl) | `{}`                                               |
| `notifiers`                               | Configure app notifiers                   | `{}`                                                    |
| `dashboardProviders`                      | Configure app dashboard providers         | `{}`                                                    |
| `dashboards`                              | Dashboards to import                          | `{}`                                                    |
| `dashboardsConfigMaps`                    | ConfigMaps reference that contains dashboards | `{}`                                                    |
| `app.ini`                             | app's primary configuration               | `{}`                                                    |
| `ldap.enabled`                            | Enable LDAP authentication                    | `false`                                                 |
| `ldap.existingSecret`                     | The name of an existing secret containing the `ldap.toml` file, this must have the key `ldap-toml`. | `""` |
| `ldap.config`                             | app's LDAP configuration                  | `""`                                                    |
| `annotations`                             | Deployment annotations                        | `{}`                                                    |
| `labels`                                  | Deployment labels                             | `{}`                                                    |
| `podAnnotations`                          | Pod annotations                               | `{}`                                                    |
| `podLabels`                               | Pod labels                                    | `{}`                                                    |
| `podPortName`                             | Name of the app port on the pod           | `app`                                               |
...
| `sidecar.dashboards.enabled`              | Enables the cluster wide search for dashboards and adds/updates/deletes them in app | `false`       |
| `sidecar.dashboards.SCProvider`           | Enables creation of sidecar provider          | `true`                                                  |
| `sidecar.dashboards.provider.name`        | Unique name of the app provider           | `sidecarProvider`                                       |
| `sidecar.dashboards.provider.orgid`       | Id of the organisation, to which the dashboards should be added | `1`                                   |
| `sidecar.dashboards.provider.folder`      | Logical folder in which app groups dashboards | `""`                                                |
| `sidecar.dashboards.provider.disableDelete` | Activate to avoid the deletion of imported dashboards | `false`                                       |
| `sidecar.dashboards.provider.allowUiUpdates` | Allow updating provisioned dashboards from the UI | `false`                                          |
| `sidecar.dashboards.provider.type`        | Provider type                                 | `file`                                                  |
| `sidecar.dashboards.provider.foldersFromFilesStructure`        | Allow app to replicate dashboard structure from filesystem.                                 | `false`                                                  |
| `sidecar.dashboards.watchMethod`          | Method to use to detect ConfigMap changes. With WATCH the sidecar will do a WATCH requests, with SLEEP it will list all ConfigMaps, then sleep for 60 seconds. | `WATCH` |
| `sidecar.skipTlsVerify`                   | Set to true to skip tls verification for kube api calls | `nil`                                         |
| `sidecar.dashboards.label`                | Label that config maps with dashboards should have to be added | `app_dashboard`                                |
...
| `sidecar.datasources.enabled`             | Enables the cluster wide search for datasources and adds/updates/deletes them in app |`false`       |
| `sidecar.datasources.label`               | Label that config maps with datasources should have to be added | `app_datasource`                               |
...
| `sidecar.notifiers.enabled`               | Enables the cluster wide search for notifiers and adds/updates/deletes them in app | `false`        |
| `sidecar.notifiers.label`                 | Label that config maps with notifiers should have to be added | `app_notifier`                               |
...
| `imageRenderer.image.repository`           | image-renderer Image repository                                                    | `app/app-image-renderer` |
...
| `imageRenderer.appProtocol`            | Protocol to use for image renderer callback url                                    | `http`                         |
| `imageRenderer.appSubPath`             | app sub path to use for image renderer callback url                            | `''`                             |
| `imageRenderer.podPortName`                | name of the image-renderer port on the pod                                         | `http`                           |
| `imageRenderer.revisionHistoryLimit`       | number of image-renderer replica sets to keep                                      | `10`                             |
| `imageRenderer.networkPolicy.limitIngress` | Enable a NetworkPolicy to limit inbound traffic from only the created app pods  | `true`                           |
| `imageRenderer.networkPolicy.limitEgress`  | Enable a NetworkPolicy to limit outbound traffic to only the created app pods   | `false`                          |
...
With app 6.3 and above
```yaml
app.ini:
  server:
    domain: monitoring.example.com
    root_url: "%(protocol)s://%(domain)s/app"
...
- extraVolumeMounts:
  - name: plugins
    mountPath: /var/lib/app/plugins
    subPath: configs/app/plugins
    existingClaim: existing-app-claim
    readOnly: false
  - name: dashboards
    mountPath: /var/lib/app/dashboards
    hostPath: /usr/shared/app/dashboards
    readOnly: false
```

## Import dashboards

There are a few methods to import dashboards to app. Below are some examples and explanations as to how to use each method:
...
dashboards:
  default:
    ...
    prometheus-stats:
      # Ref: https://app.com/dashboards/2
      gnetId: 2
      revision: 2
...
If the parameter `sidecar.dashboards.enabled` is set, a sidecar container is deployed in the app
pod. This container watches all configmaps (or secrets) in the cluster and filters out the ones with
a label as defined in `sidecar.dashboards.label`. The files defined in those configmaps are written
to a folder and accessed by app. Changes to the configmaps are monitored and the imported
dashboards are deleted/updated.

A recommendation is to use one configmap per dashboard, as a reduction of multiple dashboards inside
one configmap is currently not properly mirrored in app.
...
If the parameter `sidecar.datasources.enabled` is set, an init container is deployed in the app
pod. This container lists all secrets (or configmaps, though not recommended) in the cluster and
filters out the ones with a label as defined in `sidecar.datasources.label`. The files defined in
those secrets are written to a folder and accessed by app on startup. Using these yaml files,
the data sources in app can be imported.

Secrets are recommended over configmaps for this usecase because datasources usually contain private
data like usernames and passwords. Secrets are the more appropriate cluster resource to manage those.

Example values to add a datasource adapted from [app](http://docs.app.org/administration/provisioning/#example-datasource-config-file):
...
If the parameter `sidecar.notifiers.enabled` is set, an init container is deployed in the app
pod. This container lists all secrets (or configmaps, though not recommended) in the cluster and
filters out the ones with a label as defined in `sidecar.notifiers.label`. The files defined in
those secrets are written to a folder and accessed by app on startup. Using these yaml files,
the notification channels in app can be imported. The secrets must be created before
...
Example datasource config adapted from [app](https://app.com/docs/app/latest/administration/provisioning/#alert-notification-channels):
...
## How to serve app with a path prefix (/app)

In order to serve app with a prefix (e.g., <http://example.com/app>), add the following to your values.yaml.
...
app.ini:
  server:
    root_url: http://localhost:3000/app # this host can be localhost
```

## How to securely reference secrets in app.ini

This example uses app uses [file providers](https://app.com/docs/app/latest/administration/configuration/#file-provider) for secret values and the `extraSecretMounts` configuration flag (Additional app server secret mounts) to mount the secrets.
...
This chart supports enabling [remote image rendering](https://github.com/app/app-image-renderer/blob/master/README.md#run-in-docker)
...
By default the image-renderer pods will have a network policy which only allows ingress traffic from the created app instance

### High Availability for unified alerting

If you want to run app in a high availability cluster you need to enable
the headless service by setting `headlessService: true` in your `values.yaml`
file.

As next step you have to setup the `app.ini` in your `values.yaml` in a way
